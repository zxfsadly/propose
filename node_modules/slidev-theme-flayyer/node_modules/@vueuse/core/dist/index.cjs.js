...
function useMediaControls(target, options = {}) {
    ...
    const muted = vueDemi.ref(vueDemi.unref(options.muted) || false);
    ...
    vueDemi.watchEffect(() => {
        ...
        if (!el)
            return;
        const loop = vueDemi.unref(options.loop);
        if (loop !== undefined)
            el.loop = loop;
        const controls = vueDemi.unref(options.controls);
        if (controls !== undefined)
            el.controls = controls;
        const muted = vueDemi.unref(options.muted);
        if (muted !== undefined)
            el.muted = muted;
        const preload = vueDemi.unref(options.preload);
        if (preload !== undefined)
            el.preload = preload;
        const autoplay = vueDemi.unref(options.autoplay);
        if (autoplay !== undefined)
            el.autoplay = autoplay;
        const poster = vueDemi.unref(options.poster);
        if (poster !== undefined)
            el.poster = poster;
        const playsInline = vueDemi.unref(options.playsinline);
        if (playsInline !== undefined)
            el.playsInline = playsInline;
        const autoPictureInPicture = vueDemi.unref(options.autoPictureInPicture);
