...
import {isString, notNullish, objectMap, range, slash as slash2} from "@antfu/utils";
...
function prepareSlideInfo(data) {
  return __objSpread(__objSpread({}, data), {
...
function createSlidesLoader({data, entry, clientRoot, themeRoots, userRoot}, pluginOptions, VuePlugin, MarkdownPlugin) {
  ...
  return [
    {
      ...
      async handleHotUpdate(ctx) {
        ...
        if (ctx.file !== entryId)
          return;
        const newData = await load(entry);
        ...
        const vueModules = (await Promise.all(Array.from(hmrPages).map(async (i) => {
          ...
          return await VuePlugin.handleHotUpdate(__objSpread(__objSpread({}, ctx), {
        ...
        hmrPages.clear();
        const moduleEntries = Array.from(moduleIds).map((id) => ctx.server.moduleGraph.getModuleById(id)).filter(notNullish).concat(vueModules).filter((i) => {
          var _a2;
  ...
  async function transformMarkdown(code, pageNo, data2) {
    ...
    const frontmatter = __objSpread(__objSpread({}, ((_a = data2.headmatter) == null ? void 0 : _a.defaults) || {}), ((_b = data2.slides[pageNo]) == null ? void 0 : _b.frontmatter) || {});
...
${routes.join(",\n")}
  ...
  function generateConfigs() {
    const config = __objSpread({}, data.config);
...
function createEntryPlugin({clientRoot, themeRoots, userRoot}) {
  const mainEntry = slash3(resolve2(clientRoot, "main.ts"));
  return {
    name: "slidev:entry",
    enforce: "pre",
    async transform(code, id) {
      if (id === mainEntry) {
        ...
        async function scanStyle(root) {
          const styles = [
            join2(root, "styles", "index.ts"),
            join2(root, "styles", "index.js"),
            join2(root, "styles", "index.css"),
            join2(root, "styles.css"),
            join2(root, "style.css")
          ];
...
function createClientSetupPlugin({clientRoot, themeRoots, userRoot}) {
  const setupEntry = slash4(resolve3(clientRoot, "setup"));
  return {
    name: "slidev:setup",
    enforce: "pre",
    async transform(code, id) {
...
import {isTruthy, slash as slash5} from "@antfu/utils";
import Katex from "markdown-it-katex";

// node/plugins/setupNode.ts
import {resolve as resolve4} from "path";
import {existsSync as existsSync4} from "fs-extra";
...
async function loadSetups(roots, name, arg, initial, merge = true) {
  let returns = initial;
  for (const root of roots) {
    const path = resolve4(root, "setup", name);
    if (existsSync4(path)) {
...
function resolveShikiOptions(options) {
  ...
  return __objSpread(__objSpread({}, options), {
    themes,
...
async function createMarkdownPlugin({data: {config}, roots, mode, entry}, {markdown: mdOptions}) {
  ...
  if (config.highlighter === "shiki") {
    const {getHighlighter} = await Promise.resolve().then(() => __toModule(require("shiki")));
...
async function ViteSlidevPlugin(options, pluginOptions) {
  ...
  const VuePlugin = Vue(__objSpread({
    include: [/\.vue$/, /\.md$/],
    exclude: [],
    template: {
      compilerOptions: {
  ...
  return [
    ...
    ViteIcons(__objSpread({}, iconsOptions)),
    config.remoteAssets === true || config.remoteAssets === mode ? RemoteAssets(__objSpread({
      rules: [
        ...DefaultRules,
        {
