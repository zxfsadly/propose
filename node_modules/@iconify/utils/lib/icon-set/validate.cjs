...
function validateIconSet(obj, options) {
  ...
  } else if (typeof data.prefix !== "string" || !data.prefix.match(icon_index.matchName)) {
  ...
  if (typeof options?.provider === "string") {
    ...
    if (typeof value !== "string" || value !== "" && !value.match(icon_index.matchName)) {
      if (fix) {
  ...
  Object.keys(icons).forEach((name) => {
    if (!name.match(icon_index.matchName)) {
      if (fix) {
        delete icons[name];
        return;
      }
  ...
  if (typeof data.aliases === "object") {
    let validateAlias = function(name, iteration) {
      ...
      if (iteration > 5 || typeof item !== "object" || item === null || typeof item.parent !== "string" || !name.match(icon_index.matchName)) {
        if (fix) {
          delete aliases[name];
  ...
  Object.keys(icon_index.iconDefaults).forEach((prop) => {
    const expectedType = typeof icon_index.iconDefaults[prop];
